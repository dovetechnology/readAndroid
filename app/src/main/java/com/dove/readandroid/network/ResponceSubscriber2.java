 ...
 */
public abstract class ResponceSubscriber2<T> implements Observer<ResponseBean<T>> {
    ...
    @Override
    public void onNext(ResponseBean<T> mBaseModel) {
        if (mContext != null)
            mProgressDialog.dismiss();
        if (mBaseModel.getCode()==1) {
            onSucess(mBaseModel);//android生命周期组件里面 包装请求状态会用到

        } else {
            onFail(mBaseModel.getMessage());
        }
    }
    ...
    protected abstract void onSucess(ResponseBean<T> t);
